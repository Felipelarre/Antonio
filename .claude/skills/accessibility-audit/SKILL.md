...
description: Use when reviewing a finished or near-finished HTML/CSS page for accessibility — contrast, semantic structure, keyboard navigation, motion, and forms — following WCAG AA. Fix issues found directly.
---
...
Cover all of the following. Report every issue found, including borderline and low-severity ones, with a confidence and severity estimate. Coverage first; filtering happens at aggregation (Phase 3).
...
Aggregate all findings into a single deduplicated list and fix each issue directly. For borderline cases (e.g., contrast at 4.4:1), apply the fix anyway — accessibility is the floor, not the ceiling. If a finding is a clear false positive or out-of-scope (e.g., flagging a third-party embed you can't modify), note it and skip it.
