...
description: Use when reviewing an HTML/CSS design for visual hierarchy (what gets looked at first, second, third) and rhythm (consistent spacing/type scale, repetition with strategic variation). Fix issues found.
---

# Hierarchy and Rhythm Review

Review the current design for visual hierarchy and rhythm — the two qualities that most distinguish "intentional" from "AI-generated." **Hierarchy guides the eye** (what gets looked at first, second, third); **rhythm makes the design feel intentional** (repetition with strategic variation). When they're right, the design feels effortless to scan.

## Phase 1: Identify the surface

Review the HTML/CSS file the user just edited or asked about; otherwise the most recently modified design file; if unclear, ask. Read the file and its referenced styles, and note the medium (page / mobile / dashboard) — the rules vary by context.

## Phase 2: Review hierarchy and rhythm

Report every issue found, including uncertain and low-severity ones, with a confidence and severity estimate.
...
7. **The 5-second test.** A first-time visitor should know what to look at and what to do within 5 seconds.
...
4. **Strategic variation.** A long page breaks its pattern occasionally (background shift, wider section, centered CTA). Flag total uniformity (monotonous) and every-section-different (chaotic).
