...
description: Use at the start of new or ambiguous design work to run a structured kickoff question round before building — clarifies starting point, variations, scope, audience, and fidelity so the design is not built on missing context.
---
...
Use the `AskUserQuestion` tool — it renders native form components the user answers in a structured way. Per question:
...
`AskUserQuestion` does not return an answer immediately — after calling it, **end your turn**. Don't anticipate answers and proceed. When answers come back, read all of them, then:
