...
description: Use when finishing or reviewing an HTML/CSS page to verify every interactive element (buttons, links, inputs, toggles, nav items) has a complete set of states — default, hover, active, disabled, focus, loading — plus transitions and action feedback. Add what's missing.
---
...
Walk the design and list every interactive element: buttons (including anything with a click handler), links, form inputs, toggles (checkboxes, radios, switches), clickable cards or rows, navigation items (tabs, menu links, breadcrumbs), and custom widgets (dropdowns, accordions, modals, popovers).
...
6. **Loading** (for elements that trigger async work, e.g. a contact form submit). Disable on click to prevent double-submission, swap the label for a spinner or "Enviando…", restore on completion.
